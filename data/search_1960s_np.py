"""Newspapers.com NJ searches for early 1960s civic titles."""
...
SEARCHES = [
    (113, "wait-adams", '"George S. Adams" (Wait OR editor OR newspaper) Trenton', 1960, 1970, "Adams"),
    (76, "ncup", '"Newark Community Union" (News OR newspaper OR NCUP)', 1963, 1968, "Community Union"),
    (28, "black-newark", '"Black Newark" (Committee OR Baraka OR newspaper)', 1967, 1975, "Black Newark"),
    (41, "voice-plainfield", '"The Voice" Plainfield (Negro OR Black OR newspaper)', 1967, 1975, "Voice"),
    (70, "deliverance-voice", '"Deliverance Voice" Newark', 1966, 1975, "Deliverance"),
    (133, "freedom-reports", '"Freedom Reports" Newark', 1965, 1972, "Freedom Reports"),
    (62, "festival-arts", '"Festival of the Arts" (Yusef OR Iman OR Newark)', 1965, 1970, "Festival"),
    (69, "cricket-jihad", '"The Cricket" (Jihad OR Baraka OR "Black Music")', 1968, 1972, "Cricket"),
    (73, "utimme", '"Utimme" OR "La Voz Oculta" Trenton', 1968, 1975, "Utimme"),
]
...
def main() -> None:
    ...
    for pid, slug, keyword, y0, y1, find_term in SEARCHES:
        ...
        if not goto.get("ok"):
            ...
            continue
        send({"action": "screenshot", "name": f"q8-{slug}.png"}, timeout=60)
        ...
        for card in nj[:2]:
            ...
            shot = f"q8open-{slug}-{y}.png"
            send({"action": "screenshot", "name": shot}, timeout=60)
            report["opened"].append({"id": pid, "slug": slug, "card": card["text"], "url": card["href"], "shot": shot})
            time.sleep(0.4)
    (OUT / "q8-1960s.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
